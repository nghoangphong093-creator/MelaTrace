...
function AdminLayout(){
 ...
 <div className="admin-main"><header className="admin-top"><button className="mobile-menu" onClick={()=>setOpen(!open)}><Menu/></button><button onClick={()=>nav(-1)} className="icon-btn"><ArrowLeft/></button><div className="breadcrumb">MelaTrace <ChevronRight size={14}/> Quản trị</div><div className="admin-search"><Search/><input placeholder="Tìm mã sản phẩm, mã lô…" value={q} onChange={e=>setQ(e.target.value)}/>{results.length>0&&<div className="search-results">{results.map(x=><Link key={x.type+x.code} to={x.type==="product"?"/admin/products?code="+x.code:"/admin"} onClick={()=>setQ("")}><b>{x.code}</b><span>{x.name||x.type}</span></Link>)}</div>}</div><div className="top-user"><UserCircle/><span>{user?.name||"Admin"}<small>Administrator</small></span></div></header><div className="admin-content"><Routes>
 <Route path="/" element={<AdminDashboard/>}/><Route path="/materials" element={<CrudList title="Nguyên liệu" endpoint="materials"/>}/><Route path="/production" element={<CrudList title="Sản xuất" endpoint="production_batches"/>}/><Route path="/products" element={<AdminProducts/>}/><Route path="/tests" element={<CrudList title="Kiểm nghiệm" endpoint="tests"/>}/><Route path="/storage" element={<CrudList title="Bảo quản" endpoint="storage"/>}/><Route path="/distribution" element={<CrudList title="Phân phối" endpoint="distribution"/>}/><Route path="/cases" element={<Cases/>}/><Route path="/cases/:code" element={<CaseDetail/>}/><Route path="/analytics" element={<Analytics/>}/><Route path="/logs" element={<Logs/>}/>
 </Routes></div></div></div>
}
